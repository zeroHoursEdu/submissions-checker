...
A forkable base subject repository that gives teachers an immediately working skeleton for
authoring a new subject: an example `config.yml`, vendored `checklib`, an example assignment with
check script and fixtures, a per-subject `Dockerfile`, a `Makefile` wired to the published runner
image, pinned-runner-tag documentation, and CI that validates teacher commits. Its only coupling
to the platform is the pinned runner image; it vendors no app infrastructure.
