...
@pytest.mark.parametrize("path", ADMIN_GET_ENDPOINTS)
async def test_admin_get_teacher_forbidden(
    teacher_client: AsyncClient, path: str
) -> None:
    assert (await teacher_client.get(path)).status_code == 403


@pytest.mark.parametrize("path", ADMIN_GET_ENDPOINTS)
async def test_admin_get_student_forbidden(
    student_client: AsyncClient, path: str
) -> None:
    assert (await student_client.get(path)).status_code == 403


@pytest.mark.parametrize("path,data", ADMIN_POST_ENDPOINTS)
async def test_admin_post_anonymous_rejected(
    client: AsyncClient, path: str, data: dict
) -> None:
...
@pytest.mark.parametrize("path", ADMIN_GET_ENDPOINTS)
async def test_admin_pages_render_for_admin(
...
async def test_create_teacher_creates_active_teacher_with_hash_and_audit(
    ...
    user = (
        await db.execute(select(User).where(User.username == "newteacher"))
    ).scalar_one()
    ...
    log = (
        await db.execute(
            select(AuditLog).where(AuditLog.action == "create_teacher")
        )
    ).scalar_one()
...
async def test_create_teacher_trims_username(
    ...
    assert resp.status_code == 303
    user = (
        await db.execute(select(User).where(User.username == "spaced"))
    ).scalar_one()
    assert user.role == UserRole.TEACHER


async def test_create_teacher_short_password_rejected(
    admin_client: AsyncClient, db
) -> None:
    resp = await admin_client.post(
        "/admin/teachers/create",
...
async def test_create_teacher_duplicate_writes_no_audit(
    ...
    count = (
        await db.execute(
            select(func.count(AuditLog.id)).where(AuditLog.action == "create_teacher")
        )
    ).scalar_one()
    assert count == 0
...
async def test_toggle_active_flips_and_audits(
    ...
    target = await make_user(
        role=UserRole.TEACHER, username="victim", is_active=True
    )
    ...
    log = (
        await db.execute(
            select(AuditLog).where(AuditLog.action == "toggle_user_active")
        )
    ).scalar_one()
    assert log.actor_id == admin.id
    assert log.actor_username == admin.username
...
async def test_toggle_active_reactivates(
    admin_client: AsyncClient, make_user, db
) -> None:
    target = await make_user(
        role=UserRole.TEACHER, username="victim", is_active=False
    )
    resp = await admin_client.post(
        f"/admin/users/{target.id}/toggle-active", follow_redirects=False
    )
...
async def test_toggle_active_missing_user_returns_404(
    admin_client: AsyncClient, db
) -> None:
    resp = await admin_client.post(
        "/admin/users/999999/toggle-active", follow_redirects=False
    )
    ...
    count = (
        await db.execute(
            select(func.count(AuditLog.id)).where(
                AuditLog.action == "toggle_user_active"
            )
        )
    ).scalar_one()
    assert count == 0
...
async def test_toggle_active_self_rejected(
    admin_client: AsyncClient, admin, db
) -> None:
    """An admin cannot deactivate their own account (lockout guard)."""
    resp = await admin_client.post(
        f"/admin/users/{admin.id}/toggle-active", follow_redirects=False
    )
